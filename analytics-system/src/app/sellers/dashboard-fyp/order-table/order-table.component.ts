...
export class OrderTableComponent implements OnInit {
  ...
  displayedColumns: string[] = ['orderId', 'customerName', 'productName', 'date', 'orderAmmount', 'revenue'];//'status'
  dataSource;
  ...
  getOrders(orders) {
    var newList = []
    for (let a of orders){
      newList.push({orderId: a['_id'], customerName: a['buyerUserName'], orderAmmount: a['quantity'], revenue: a['amountPaid'], date: a['orderDate'], productName: a['productName']})
    }
    // console.log(orders)
    // console.log(newList)
